...
from stakeroute.model.protocol import Accepted, ModelClient, RejectionReason, Rejected
from stakeroute.storage.repository import Repository


def compute_interaction_id(
    tenant_id: str, purpose: str, prompt: str, requested_at_ms: int
) -> str:
    """``sha256(tenant|purpose|request_hash|requested_at_ms)`` (data-model.md),
    mirroring ``core/types.py::compute_event_id``'s pattern for events."""
    request_hash = hashlib.sha256(prompt.encode("utf-8")).hexdigest()
    key = f"{tenant_id}|{purpose}|{request_hash}|{requested_at_ms}"
...
class ModelInteractionRecorder:
    ...
    async def complete[T](
        ...
        interaction_id = compute_interaction_id(
            self._tenant_id, purpose, prompt, requested_at_ms
        )
        ...
        if accepted:
            return Accepted(
                value=value, interaction_id=interaction_id, latency_ms=raw_result.latency_ms
            )
        return Rejected(
